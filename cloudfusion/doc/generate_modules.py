...
def write_heading(module, kind='Module'):
    ...
    module = module.title()
    #heading = title_line(module + ' Documentation', '=')#my addition
    #heading += 'This page contains the %s %s documentation.\n\n' % (module, kind)#my addition
    heading = ""#my addition
    return heading
    ...
    for root, subs, files in tree:
        ...
        if root == path:
            ...
            for py_file in py_files:
                ...
                #and is not excluded (my addition)
                if check_for_code(os.path.join(path, '%s.py' % module)) and not os.path.join(path, '%s.py' % module) in excludes:
